...
__hypeR__ is an R package focused on the downstream analysis of geneset enrichment workflows. Because there are many upstream tools for performing enrichment, users may opt for those while also planning to use the visualization, summarization, and reporting features offered by __hypeR__ This task is straightforward given the object oriented nature of __hypeR__
...
### Signature
Start with a differential expression dataframe.
```{r}
...
Rank genes by upregulation and extract a weighted signature.
```{r}
df.up <- df %>% filter(t > 0 & fdr < 0.001) %>% arrange(desc(t))
...
To wrap third-party packages, we create modified version of the `hypeR()` function that calls external packages when performing the enrichment analysis, and then coerces the results into a `hyp` or `multihyp` object. We will wrap additional packages based on user demand.
```{r}
...
### *hypeR* functionality
The resulting `hyp` or `multihyp` objects are compatible with all __hypeR__ functionality.
```{r}
