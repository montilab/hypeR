...
# Simply a character vector of symbols
signature <- c("GENE1", "GENE2", "GENE3")

# A pre-ranked character vector of symbols
ranked.signature <-  c("GENE1", "GENE2", "GENE3")

# A pre-ranked named numerical vector of symbols with ranking weights
weighted.signature <-  c("GENE1"=1.22, "GENE2"=0.94, "GENE3"=0.77)

```

#### Geneset
A geneset is simply a list of vectors, therefore, one can use any custom geneset in their analyses, as long as it is appropriately defined. In our tutorials, we will use genesets from [REACTOME](https://reactome.org/). There is also what is called relational genesets, whereby genesets are organized into a hiearchy, we will explore these in later tutorials. 
```{r}
...
Using a differential expression dataframe created with limma, we will extract an signature of upregulated genes for use with a hypergeometric test and rank genes descending by their differential expression level for use with a kstest. We'll also import a genesets from [REACTOME](https://reactome.org/).
```{r}
