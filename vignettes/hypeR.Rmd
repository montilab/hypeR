...
hypeR::download_msigdb(species="Homo sapiens") 
...
## Loading Gene Sets
Use hypeR::db_get() to retrieve a downloaded geneset. By default, hypeR comes pre-loaded with the following genesets. In this example, we are interested in all three, therefore we concatenate them. A geneset is simply a list of character vectors, therefore, one can use any custom geneset in their analysis, as long as it is appropriately defined.
```{r}
