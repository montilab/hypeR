...
### Dots plot
One can visualize the top enriched pathways using `hyp_dots()` which returns a horizontal dots plot. Each dot is a geneset, where the color represents the significance and the size signifies the geneset size.
```{r, fig.height=5}
...
All workflows begin with performing hyper enrichment with `hypeR()`. Note that when passing an `rgsets` object, ensure `is_rgsets=TRUE`. Otherwise, `hypeR()` will treat it as a list of unstructured gsets. 
```{r}

hyp_obj <- hypeR(ranked.signature, rgsets, test="kstest", is_rgsets=TRUE, fdr_cutoff=0.01)

```

### Hierarchy map
One can visualize the top enriched pathways using `hyp_hmap()` which will return a hiearchy map. Each node represents a geneset, where the shade of the gold border indicates the normalized significance of enrichment. Hover over the leaf nodes to view the raw value. Double click internal nodes to cluster their first degree connections. Edges represent a directed relationship between genesets in the hiearchy.
```{r, fig.height=7}

hyp_hmap(hyp_obj, top=30, return_plots=TRUE)
