...
test_that("KS Test is working", {
  ...
  expect_equal(hyp_obj$data %>% filter(label == genesets_names[[1]]) %>% pull(hits) %>% unname,
               paste0("'", genesets[[1]][1:5], "'",collapse=","))

  # Geneset 2 is mixed
  experiment <- c(head(genesets[[2]], 8), LETTERS, tail(genesets[[2]], 10))
  hyp_obj <- hypeR(experiment, genesets, background=2522, test="kstest")
  expect_equal(hyp_obj$data %>% filter(label == genesets_names[[2]]) %>% pull(hits) %>% unname,
               paste0("'", genesets[[2]][1:8], "'",collapse=","))

  # Geneset 3 is bottom skewed
  experiment <- c(head(genesets[[3]], 1), LETTERS, tail(genesets[[3]], 8))
  hyp_obj <- hypeR(experiment, genesets, background=2522, test="kstest")
  expect_equal(hyp_obj$data %>% filter(label == genesets_names[[3]]) %>% pull(hits) %>% unname,
